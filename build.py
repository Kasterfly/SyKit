...
RUNTIME_LOCK_PATH = TOOL_DIR / "requirements.lock"
PYTHON_IMAGE = (
...
def load_config(config_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(extensions, dict):
        raise BuildError('The "extensions" setting must be an object.')
    return config
...
def generate_dockerfile(port: int) -> str:
    return (
        ...
        "-r requirements.txt\n"
        "RUN groupadd --system sykit && useradd --system --gid sykit "
        "--home-dir /app sykit\n"
        "COPY --chown=sykit:sykit . .\n"
        "USER sykit\n"
        f"EXPOSE {port}\n"
        'CMD ["python", "main.py"]\n'
    )
...
def prepare_staging(
    ...
    shutil.copy2(RUNTIME_LOCK_PATH, STAGING_DIR / "requirements.txt")
