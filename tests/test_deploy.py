...
class DockerFileGenerationTests(unittest.TestCase):
    def test_dockerfile_content(self) -> None:
        ...
        self.assertIn("FROM python:3.12.13-slim-trixie@sha256:", content)
        self.assertIn("EXPOSE 8443", content)
        self.assertIn("--require-hashes -r requirements.txt", content)
        self.assertIn("USER sykit", content)
        self.assertIn("COPY --chown=sykit:sykit", content)
